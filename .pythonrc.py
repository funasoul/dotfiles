"""
https://gist.github.com/taavi223/1340876
...
try:
    ...
    try:
        ...
        import rlcompleter
        # https://gist.github.com/taavi223/1340876#gistcomment-3098449
                ...
                for filename in os.listdir(self.directory):
                    ...
                    try:
                        ...
                        if base_modulepath != base_filepath:
                            sys.stderr.write(
                                "%sThe scratchpad file '%s' "
                                "conflicts with a built-in "
                                "module.%s\n" % (_c["LightRed"], filename, _c["Normal"])
                            )
                        else:
                            globals()[modulename] = module
                            verb = imported and "Imported" or "Reloaded"
                            sys.stdout.write(
                                "%s%s the scratchpad file "
                                "'%s'.%s\n"
                                % (_c["LightGreen"], verb, filename, _c["Normal"])
                            )
                    ...
                    # the modules, so that they can correct the errors.
                        ...
                        sys.stderr.write(
                            "%sUnable to %s the scratchpad file "
                            "'%s'.%s" % (_c["LightRed"], verb, filename, _c["Normal"])
                        )
